...
echo "Build your first network (BYFN) using marbles and couchdb end-to-end test"
echo
CHANNEL_NAME="$1"
DELAY="$2"
...
instantiateChaincode () {
	...
	if [ -z "$CORE_PEER_TLS_ENABLED" -o "$CORE_PEER_TLS_ENABLED" = "false" ]; then
		peer chaincode instantiate -o orderer.example.com:7050 -C $CHANNEL_NAME -n marbles -v 1.0 -c '{"Args":["init"]}' -P "OR ('Org1MSP.member','Org2MSP.member')" >&log.txt
	else
...
echo
echo "========= All GOOD, BYFN using marbles and couchdb execution completed =========== "
echo
